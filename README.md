...
- 固定 Cowork 和 Code 模型菜单：`Opus 4.8` 是伪装入口，`Kimi-k2.6` 是真实入口。
- Cowork 和 Code 均显示五档强度：`低 / 中 / 高 / 超高 / 最大`，默认模型为 `Opus 4.8`，默认强度为 `最大`。
...
如果 Code 模式报 `401 API Key invalid`，或者出现“Cowork 可以用、Code 不能用”，只需要双击运行：

```bash
repair_code_runtime.command
```

它不需要 sudo，不替换 `/Applications/Claude.app`。脚本现在会先保留故障现场：读取正在运行的桌面版 Code 子进程、实际 `--model`、是否继承了 `ANTHROPIC_BASE_URL` / `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_API_KEY`，并扫描最近 transcript 里的 `401`、`403`、token limit 和 policy block。取证完成后才退出 Claude、终止旧 Code 子进程、同步当前“配置第三方推理”里的网关环境，并扫描最近 Code 会话项目里的 `.env*` 和 `.claude/settings*.json` 是否覆盖了旧网关或旧 Key。同步共享的 `~/.claude/settings.json` 前会先备份，写入时只合并必要的 `env` 字段，不删除终端 CLI、VS Code 插件、hooks、statusLine、enabledPlugins 等已有配置。
...
- `Opus 4.8` 是显示兼容入口；Code/Cowork 实际启动 Claude Code CLI、切换 session model、写入 `session_context.model` 时会转成 provider 真实模型，例如当前的 `kimi-for-coding`。上下文窗口不能按显示名伪装，必须跟随网关返回的真实模型能力。安装脚本会优先读取手动模型列表或 `/v1/models` 的第一项作为 Claude Code 默认真实模型，并把 provider 返回的上下文窗口同步到 Claude Code 运行时配置。
- `Kimi-k2.6` 是真实入口，优先使用网关返回的真实 Kimi id，找不到时兜底为 `kimi-for-coding`。
- Cowork 和 Code 都必须固定显示两个模型入口，并显示五档强度：`低 / 中 / 高 / 超高 / 最大`，默认是 `Opus 4.8 · 最大`。
...
- `repair_code_runtime.command` 会先写 `runtime.pre_repair_active_code_processes`、`runtime.pre_repair_active_code_model`、`runtime.pre_repair_active_code_env` 和 `runtime.recent_code_api_errors`，再退出 Claude。这样其他电脑再出现“repair 显示正常但桌面版仍 401”时，日志能区分：Key 真无效、活动子进程还在用旧官方模型、活动子进程没继承 env、项目覆盖、还是启动层补丁静态命中但实际路径没走到。
...
- 出现异常时先运行对应入口：安装问题跑 `install.command`，Code 401 跑 `repair_code_runtime.command`，官方覆盖安装问题跑 `prepare_official_update.command`。三个入口都会生成 `Logs/latest.json` 和 `Logs/command-latest.log`，比截图更容易定位是哪一个补丁点失效。
...
7. 安装原生菜单语言资源。
8. 替换部分硬编码英文文案。
9. 为第三方网关补充 Opus 默认模型识别兼容，界面显示 `Opus 4.8`，真实 Claude Code 默认模型优先跟随 provider 模型列表。
...
Code 页面模型菜单固定为两项：`Opus 4.8` 和 `Kimi-k2.6`。`Opus 4.8` 是显示兼容入口，但它不会再作为 `--model opus` 传给 Claude Code；补丁会把启动参数和 session model 转成 provider 真实模型。`Kimi-k2.6` 是真实模型入口，会优先使用网关返回的真实 Kimi model id，未返回时使用 `kimi-for-coding` 作为实际 id。上下文窗口以真实请求 model 和网关返回能力为准，不以显示名为准。

切换到 `Kimi-k2.6` 后，`Opus 4.8` 仍会保留在菜单第一项，不会被挤掉，也不会再出现两个 Kimi。切回 `Opus 4.8` 后，Kimi 也会继续保留。
...
Cowork 页面同样固定显示两项模型和五档强度。默认值固定为 `Opus 4.8 · 最大`；旧缓存里的 `kimi-for-coding`、`Kimi-k2.6` 或 `cowork_effort_level=high` 不再决定初始默认。手动切换 `Kimi-k2.6` 后当前界面仍会生效，切回 `Opus 4.8` 不会清空强度菜单。

Claude Desktop 更新后需要重新运行补丁。每次适配新版都会重点回归 Cowork 和 Code 两个模型菜单：不能出现 `Legacy Model`，不能只剩 `· 高` / `· 最大` 这类空模型按钮，默认必须是 `Opus 4.8 · 最大`，`Kimi-k2.6` 必须可选，五档强度必须都能点击。
...
如果已经打开过旧会话，旧会话文件里可能保存了 `model: "opus"`、`model: "opus[1m]"`、`model: "default"` 或 `model: "claude-sonnet-4-6"` 这类官方模型值。安装脚本会在能发现真实 provider 默认模型时，把这些已保存会话迁移到真实模型 id，并在安装时终止旧的 Claude Code / disclaimer 子进程；否则正在运行的旧子进程仍可能绕开第三方 provider 默认模型，继续按官方模型发起请求。
...
这说明请求已经到达第三方网关，但当前电脑保存的凭据不可用，通常是 API Key 没复制、复制错了、过期了，或认证方式选错。先运行 `repair_code_runtime.command`，然后看 `Logs/latest.json`：
...
- `runtime.gateway_auth_check` 和 `runtime.gateway_messages_auth_check` 都通过，但 Code 仍 401：先看 `runtime.pre_repair_active_code_model`。如果里面有 `default`、`claude-sonnet-*`、`claude-opus-*` 或 `stale=true`，说明旧 Code 会话模型还在绕开第三方 provider 默认模型；运行最新版 `install.command` 或 `repair_code_runtime.command` 会迁移这些会话并终止旧子进程。否则继续看 `runtime.claude_code_gateway_env`，确认当前第三方推理配置是否已经同步到 Claude Code CLI 的 `ANTHROPIC_BASE_URL` / `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_API_KEY`。同步会保留共享 settings 里的 CLI/VS Code 配置，并在 `runtime.desktop_code_env` / `runtime.terminal_cli_env` 中说明桌面版和终端 CLI 哪一边异常。
...
Code 页上下文窗口有两条来源：历史消息里的 `## Context Usage` 文本，以及底部实时上下文弹窗里的 `contextUsage.rawMaxTokens`。官方值可能因为 `opus[1m]` 显示入口写成 `1.0M`，也可能因为旧运行时配置写成 `200.0k`，百分比还可能被钳制成 `100%`。安装脚本会把 provider 返回的真实窗口同步到 Claude Code 运行时；如果新电脑没有 `.claude.json`，会创建最小配置写入真实窗口。同时会注入前端解析器和实时弹窗组件，显示分母和百分比都按真实窗口重算。
