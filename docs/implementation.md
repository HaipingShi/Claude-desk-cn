...
- 2026-05-20 新版汉化复查新增两类记录：一类是第三方推理设置页中由 `ion-dist/i18n/zh-CN.json` 和 `defaultMessage` 共同控制的英文残留；另一类是 macOS 原生菜单 `Localizable.strings` 漏项，例如“查看”菜单中的 `Background tasks`。后者不在前端 bundle 中，必须通过 `i18n.localizable_menu_labels` 检测，不能只看 `i18n.known_missing_strings`。
